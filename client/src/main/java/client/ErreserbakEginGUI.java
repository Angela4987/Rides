...
public class ErreserbakEginGUI extends JFrame {
	...
	// ALDATU Date -> XMLGregorianCalendar
	...
	public ErreserbakEginGUI(final Bidaiaria u)				// GEHITU
	{
		...
		jComboBoxDestination.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				...
				// ALDATU jCalendar1.getDate() -> toXMLGregorianCalendar(jCalendar1.getDate())
				datesWithRidesCurrentMonth=facade.getThisMonthDatesWithRides((String)jComboBoxOrigin.getSelectedItem(),(String)jComboBoxDestination.getSelectedItem(),toXMLGregorianCalendar(jCalendar1.getDate()));
			...
			public void propertyChange(PropertyChangeEvent propertychangeevent)
			{
				...
				else if (propertychangeevent.getPropertyName().equals("calendar"))
				{
					...
					try {
						...
						// ALDATU UtilDate.trim(jCalendar1.getDate()) -> toXMLGregorianCalendar(UtilDate.trim(jCalendar1.getDate()))
						List<Ride> rides=facade.getRides((String)jComboBoxOrigin.getSelectedItem(),(String)jComboBoxDestination.getSelectedItem(),toXMLGregorianCalendar(UtilDate.trim(jCalendar1.getDate())));
						...
						for (Ride ride:rides){
							// ALDATU ride.getnPlaces() -> ride.getNPlaces()
							if (ride.getNPlaces() > 0 && ride.getEgoera().equals("martxan")) {	
								Vector<Object> row = new Vector<Object>();
								// ALDATU  row.add(ride.getDriver().getName()) -> row.add(((User)ride.getDriver()).getIzena())
								row.add(((User)ride.getDriver()).getIzena());
								row.add(ride.getNPlaces());		// ALDATU ride.getnPlaces() -> ride.getNPlaces()
		...
		btnErreserbatu.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) { // Eserleku kantitatea begiratu behar da. 
				if(tableRides.getSelectedRow()!=-1) {
					...
					BLFacade facade = MainGUI.getBusinessLogic();	
					float prezioa = Float.parseFloat(tableRides.getValueAt(bidai_errenkada,2).toString());		// ALDATU float -> Float.parseFloat()
					int bidaiZenbaki = Integer.parseInt(tableRides.getValueAt(bidai_errenkada,3).toString());		// ALDATU int -> Integer.parseInt()
					try {
...
//	}
	...
	public static void paintDaysWithEvents(JCalendar jCalendar, List<XMLGregorianCalendar> datesWithEventsCurrentMonth, Color color) {
	    ...
	    for (XMLGregorianCalendar xmlDate : datesWithEventsCurrentMonth) {
	    	
	        Calendar eventCalendar = xmlDate.toGregorianCalendar();
	        
	        if (eventCalendar.get(Calendar.MONTH) == month && eventCalendar.get(Calendar.YEAR) == year) {
	            Component o = (Component) jCalendar.getDayChooser().getDayPanel()
	                    .getComponent(eventCalendar.get(Calendar.DAY_OF_MONTH) + offset);
	            o.setBackground(color);
	        }
	    }
