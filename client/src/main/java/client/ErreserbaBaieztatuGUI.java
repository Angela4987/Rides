...
public class ErreserbaBaieztatuGUI extends JFrame {
	 ...
	 * Create the frame.
	 */
	public ErreserbaBaieztatuGUI(final Bidaiaria u) {		// GEHITU final
		...
		btnBaieztatu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tableErreserbaOnartuak.getSelectedRow() != -1) {
					...
					int erreserbaZenbaki = Integer.parseInt(tableErreserbaOnartuak.getValueAt(bidai_errenkada,6).toString());	// ALDATU int -> Integer.parseInt()
			...
			for (Erreserba e : erreserbak) {
				if(e.getEgoera().equals("onartu")) {
					Object[] row = {
							...
							e.getNPlaces(),			// ALDATU e.getnPlaces() -> e.getNPlaces()
							e.getBookNumber()
					};
					model.addRow(row);
				}
