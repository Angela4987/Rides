...
public class ErreklamazioaSortuGUI extends JFrame {
	...
	public ErreklamazioaSortuGUI(final Bidaiaria b) {			// GEHITU final
		...
		final JLabel lblMezua = new JLabel(""); //$NON-NLS-1$ //$NON-NLS-2$
		...
		comboErreserba.setBounds(173, 39, 97, 21);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if(comboErreserba.getSelectedItem() != null){
						Integer erreserbazbk = Integer.parseInt(comboErreserba.getSelectedItem().toString());// ALDATU int -> Integer.parseInt()
		...
		if (erreserbak != null && !erreserbak.isEmpty()) {
			for (Erreserba e : erreserbak) {
				...
				if(bid.getEgoera().equals("amaituta") && e.getEgoera().equals("amaituta")) {
						...
						ErreserbaBox.addElement(Integer.parseInt(e.getBookNumber()));		// GEHITU Integer.parseInt
