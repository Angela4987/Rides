...
public class BidaiaKantzelatuGUI extends JFrame {
	 ...
	 * Create the frame.
	 */
	public BidaiaKantzelatuGUI(final Driver u) {				// GEHITU final
		...
		btnKantzelatu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				facade.bidaiKantzelatu(u,Integer.parseInt(model.getValueAt(tableBidaiak.getSelectedRow(),5).toString()));	// ALDATU int Integer.parseInt()
		...
		if (bidaiak != null && !bidaiak.isEmpty()) {
			for (Ride r : bidaiak) {
				Object[] row = {
						...
						r.getDate(),
						r.getPrice(),
						r.getNPlaces(),		// ALDATU r.getnPlaces() -> r.getNPlaces()
						r.getRideNumber(),
						r.getEgoera()
				};
				model.addRow(row);
