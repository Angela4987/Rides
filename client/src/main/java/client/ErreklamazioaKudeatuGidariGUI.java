...
public class ErreklamazioaKudeatuGidariGUI extends JFrame {
	 ...
	 * Create the frame.
	 */
	public ErreklamazioaKudeatuGidariGUI(final Driver g) {			// GEHITU final
		...
		btnOnartu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tableErreklamazioak.getSelectedRow() != -1) {
					...
					int errekZenb = Integer.parseInt(tableErreklamazioak.getValueAt(bidai_errenkada,0).toString());//ALDATU int -> Integer.parseInt()
					try {
						facade.egoeraEzarri(errekZenb,"onartu");
					} catch (ErreklamazioaEbatzitaException_Exception e1) {
		...
		tableErreklamazioak.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(tableErreklamazioak.getSelectedRow() != -1) {
					int bidai_errenkada = tableErreklamazioak.getSelectedRow();
					int erreserbaZenbaki = Integer.parseInt(tableErreklamazioak.getValueAt(bidai_errenkada,0).toString());//ALDATU int -> Integer.parseInt()
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if(tableErreklamazioak.getSelectedRow() != -1) {
						BLFacade facade = MainGUI.getBusinessLogic();
						int bidai_errenkada = tableErreklamazioak.getSelectedRow();
						int erreklamaziozenb = Integer.parseInt(tableErreklamazioak.getValueAt(bidai_errenkada,0).toString());//ALDATU int -> Integer.parseInt()
	...
	private void loadErreklamazioak(Driver g) {
		...
		if (erreklamazioak != null && !erreklamazioak.isEmpty()) {
			...
			if (model.getRowCount() > 0) {
				...
				int bidai_errenkada = tableErreklamazioak.getSelectedRow();
				int erreklamazioZenbaki = Integer.parseInt(tableErreklamazioak.getValueAt(bidai_errenkada,0).toString());//ALDATU int -> Integer.parseInt()
