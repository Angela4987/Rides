...
public class EskaeraGUI extends JFrame {
	...
	public EskaeraGUI(final Driver u) {					// GEHITU final
		...
		btnOnartu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tableEskaerak.getSelectedRow() != -1) {
					...
					facade.erreserbaOnartu(Integer.parseInt(model.getValueAt(tableEskaerak.getSelectedRow(),6).toString()),u);// ALDATU int -> Integer.parseInt()
		...
		btnDeuseztatu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tableEskaerak.getSelectedRow() != -1) {
					BLFacade facade = MainGUI.getBusinessLogic();
					facade.erreserbaDeuseztatu(Integer.parseInt(model.getValueAt(tableEskaerak.getSelectedRow(),6).toString()));// ALDATU int -> Integer.parseInt()
					model.removeRow(tableEskaerak.getSelectedRow());
				}		
			}
		});
		...
		if (reservedRides != null && !reservedRides.isEmpty()) {
			for (Erreserba erreserba : reservedRides) {
				Object[] row = {
						...
						erreserba.getNPlaces(),			// ALDATU getnPlaces() -> getNPlaces()
